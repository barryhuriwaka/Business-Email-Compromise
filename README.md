...
## Executive Summary
A Business Email Compromise (BEC) attempt targeted a finance employee through CFO impersonation. The attacker attempted to initiate fraudulent financial activity and created a suspicious inbox rule to hide incoming messages. Investigation confirmed failed MFA attempts from Nigeria, malicious inbox rule creation and a spoofed email designed to pressure the user into urgent payment action.
...
+ Email impersonating the CFO requesting urgent payment
...
## Likely Attack Chain
+ Attacker obtained credentials (phishing or breach dump)

+ Attempted login from Nigeria

+ Triggered MFA fatigue to trick the user

+ Created an inbox rule to hide replies

+ Sent a fraudulent payment request
